...
function submitDailyDoubleWager() {
  //still having score error concatenating strings
  dailyDouble = new DailyDouble();
  dailyDouble.validateWager(ddWagerInput.value, currentRoundObj, game.playerTurn, wagerErrorText);
...
function submitFinalWagers() {
  ...
  let dailyDoubleObjs = wagerInputs.map((wager, index) => {
    ...
    let isValid = currentRoundObj.validateFinalWager(newObj, parseInt(wager.value), game.players[index], errorTextElements[index]);
    if (isValid) {
      allValid++
    }
    return newObj;
...
function startGame() {
  ...
  //move this to game (property on Game) game.players
